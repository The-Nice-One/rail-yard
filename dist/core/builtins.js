 ...
 */
export function registerBuiltins(registry, sourceDir) {
    // ── \textinput{path} ──────────────────────────────────────────────────────
    ...
    registry.registerDefault('textinput', async (path) => {
    ...
    registry.registerDefault('textinputraw', async (path) => {
        const fullPath = resolve(sourceDir, path.trim());
        if (!existsSync(fullPath)) {
    ...
    registry.registerDefault('note', (content) => {
        return `> **Note:** ${content}`;
    });
    // ── \warn{content} ────────────────────────────────────────────────────────
    registry.registerDefault('warn', (content) => {
        return `> **⚠ Warning:** ${content}`;
    });
    // ── \details{summary}{content} ────────────────────────────────────────────
    // Renders an HTML <details>/<summary> collapsible block.
    registry.registerDefault('details', (summary, content) => {
        return [
            `<details>`,
    ...
    registry.registerDefault('html', (content) => content);
    // ── \newcommand{name}{body} ───────────────────────────────────────────────
