...
export class Site {
    ...
    async build() {
        ...
        const pagesWritten = this.pages.length -
            errors.filter((e) => this.pages.some((p) => p.sourcePath === e.file))
                .length;
        console.log();
        if (errors.length === 0) {
            console.log(pc.green(pc.bold(`✓ Done`)) +
                pc.dim(` in ${durationMs}ms — ${this.pages.length} pages, ${cssWritten} CSS, ${staticCopied} static`));
        }
        else {
            console.log(pc.yellow(pc.bold(`⚠ Done with ${errors.length} error(s)`)) +
