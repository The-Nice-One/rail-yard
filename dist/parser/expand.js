 ...
 */
export async function expand(nodes, registry, options = {}, depth = 0) {
    const { sourcePath = '<unknown>', maxDepth = 16 } = options;
    if (depth > maxDepth) {
        throw new Error(`Command expansion exceeded maximum depth (${maxDepth}) in ${sourcePath}. ` +
    ...
    for (const node of nodes) {
        switch (node.type) {
            ...
            case 'command': {
                ...
                const expandedArgs = await Promise.all(node.args.map(argNodes => expand(argNodes, registry, options, depth)));
                ...
                try {
                    ...
                    throw new Error(`Command \\${node.name} threw an error in ${sourcePath}: ` +
            ...
            default: {
                ...
                throw new Error(`Unhandled AST node type: ${_exhaustive.type}`);
            }
        }
    }
