...
async function runBuild(args, watch) {
    const configPath = getArg(args, "--config", "-c") ?? findConfig();
    if (!configPath) {
        console.error(pc.red("✗ No build config found. Create build.ts or rail-yard.config.ts, or use --config."));
        process.exit(1);
    }
...
async function runPreprocess(args) {
    ...
    if (!inputDir || !outputDir) {
        ...
        console.log(pc.dim("  Example: rail-yard preprocess -i ./docs -o ./dist-wiki"));
        process.exit(1);
    }
...
function findConfig() {
    ...
    for (const c of candidates) {
        if (existsSync(join(process.cwd(), c)))
            return join(process.cwd(), c);
    }
    return undefined;
}
