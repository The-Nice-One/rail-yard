/**
 * commark test suite — parser + expander
 *
...
import type { CommandNode, TextNode, CodeFenceNode, InlineCodeNode } from '../src/types.js'
...
function expectNotContains(actual: string, substring: string): void {
  if (actual.includes(substring)) {
    throw new Error(`Expected string NOT to contain: ${JSON.stringify(substring)}\n    Got: ${JSON.stringify(actual)}`)
  }
}
...
async function expandStr(input: string, registry = makeRegistry()): Promise<string> {
...
await test('command handler error is wrapped with context', async () => {
  const r = new CommandRegistry()
  r.register('boom', () => { throw new Error('handler exploded') })
  try {
