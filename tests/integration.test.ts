/**
 * rail-yard integration tests — full pipeline
 *
...
function makeTempDir(): string {
  const dir = join(
    tmpdir(),
    `rail-yard-test-${Date.now()}-${Math.random().toString(36).slice(2)}`,
  );
...
await test("\\details renders as HTML details element", async () => {
  ...
  try {
    writeFixture(
      tmp,
      "pages/d.md",
      String.raw`\details{Click me}{Hidden content}`,
    );
...
await test("addFolder discovers all .md files", async () => {
  ...
  try {
    ...
    if (!existsSync(join(tmp, "dist/a.html")))
      throw new Error("a.html not found");
    if (!existsSync(join(tmp, "dist/b.html")))
      throw new Error("b.html not found");
    if (!existsSync(join(tmp, "dist/sub/c.html")))
      throw new Error("sub/c.html not found");
  } finally {
    cleanup(tmp);
  }
});
...
await test("build continues after one page error", async () => {
  ...
  try {
    ...
    if (!existsSync(join(tmp, "dist/good.html")))
      throw new Error("good.html not written");
